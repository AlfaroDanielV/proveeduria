...
- `docs/GUIA_AMBIENTE_LOCAL.md`: levantar y validar TODO localmente (Postgres+Azurite+api+worker+portal, WhatsApp simulado con curl firmado) — comandos verificados.
- `docs/GUIA_DEPLOYMENT_DEMO.md`: publicar el demo para Atemporal (VM+systemd+nginx, config completa, Meta, checklist de entrega, requisitos mínimos y costos).
